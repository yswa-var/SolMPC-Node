...
func main() {
	...
	fmt.Println("ARG:", id)
	validators, err := loadValidators("/Users/apple/Documents/GitHub/tilt-validator-main/data/validators.csv") // Load validators from the CSV file
	if err != nil {
	...
	fmt.Println("a5678976897689------------------ signing")
	msgToSign := []byte(utils.GenerateTransactionHash()) // Raw message
	digestMsg := mpc.Digest(msgToSign)                   // Hash it before signing
	...
	go func() {
		...
		sign, err = mpcParty.Sign(ctx, digestMsg) // Use digestMsg
		if err != nil {
